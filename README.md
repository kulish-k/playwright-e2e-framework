...
* **Repeated login slowing execution:** Employs global session handling via Playwright's *Project Dependencies*. A dedicated `setup` worker navigates the UI authentication strictly once, serializes the browser context, and hydrates the `auth.json`. Future workers inherit this authorized token footprint blindly, reducing test execution times by upward of 80%.
